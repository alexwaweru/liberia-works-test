...
export function LoginForm({ next }: { next?: string }) {
  ...
  async function onEmailSubmit(data: EmailFormData) {
    const result = await loginAction({ email: data.email, password: data.password }, next)
    if (result?.error) toast.error(result.error)
  }

  async function onPhoneSubmit(data: PhoneFormData) {
    const result = await loginAction({ phoneNumber: data.phoneNumber, password: data.password }, next)
    if (result?.error) toast.error(result.error)
  }
