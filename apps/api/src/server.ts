...
import { serve as inngestServe } from 'inngest/fastify'

import { env } from './config/env.js'
import { inngest, inngestFunctions } from './lib/inngest/index.js'
...
/**
 * Build and configure the Fastify app. Used by both `src/start.ts` (local dev,
 * which calls `app.listen()`) and `api/index.ts` (the Vercel serverless entry,
 * which delegates raw requests to `app.server.emit('request', …)`).
 */
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // All background functions live in src/lib/inngest/. The Inngest cloud (or
  // the local dev server) invokes this endpoint to discover and run them.
  app.route({
    method: ['GET', 'POST', 'PUT'],
    url: '/api/inngest',
    schema: { hide: true },
    handler: inngestServe({
      client: inngest,
